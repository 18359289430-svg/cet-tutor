...
if (pathname === '/api/essay/upload' && req.method === 'POST') {
    ...
    if (false && req.url === '/webhook/deploy') {
        ...
        // POST /api/tts/generate - 听力训练TTS音频生成
        if (pathname === '/api/tts/generate' && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString(); });
            req.on('end', async () => {
                try {
                    const { text, exam_type } = JSON.parse(body);
                    if (!text || text.length < 10) {
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ error: 'Text too short' }));
                        return;
                    }
...
function buildRagContext(userMessage, personality, weakDims, dimScores, wrongSummary, studyDays, skillLevels, diagTrend, diagCount) {
    ...
    if (skillLevels) {
        try {
            ...
            var lvLabels = { 1: '薄弱(需加强)', 2: '待提升(需练习)', 3: '良好(可巩固)', 4: '冲刺(可拔高)' };
            ...
            context += '\n→ 参考估算: ' + totalEst + '/710，' + (passGap > 0 ? '距425参考线约差' + passGap + '分，建议重点补薄弱项' : '约超425参考线' + (-passGap) + '分，巩固优势');
            ...
            if (weakestSkill) {
                var focusMap = { listening: '听力最薄弱，先出短对话练关键词，逐步加长', reading: '阅读最薄弱，先出细节定位题，再加推理题', writing: '写作最薄弱，先练翻译句子，再写段落', translation: '翻译最薄弱，先练关键词翻译，再练完整句' };
                context += '\n→ 最弱技能: ' + (lvMap[weakestSkill] || weakestSkill) + '，' + (focusMap[weakestSkill] || '从基础练起');
            }
        } catch(e) {}
    }
    ...
    if (diagTrend && diagTrend.length > 0) {
        context += '\n- 进步趋势: ' + diagTrend + ' → 有进步要鼓励"继续保持"，退步维度重点练"这块还需要多练"';
    }
    if (diagCount && diagCount > 1) {
        context += '\n- 已完成诊断: ' + diagCount + '次';
        if (diagCount >= 3) context += '（多次诊断用户，可适当增加真题难度练习）';
    }
...
async function handleDeepseekEssayGrade(req, res) {
    ...
    if ((pathname.startsWith('/css/') || pathname.startsWith('/js/'))) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            if (acceptEncoding.includes('gzip') && fs.existsSync(gzPath)) {
                res.setHeader('Cache-Control', 'public, max-age=60, s-maxage=0');
    ...
    if (fs.existsSync(publicDir) && pathname.startsWith('/public/')) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            res.setHeader('Cache-Control', ext === '.json' ? 'no-cache' : 'public, max-age=60, s-maxage=0');
            res.setHeader('Content-Type', contentTypes[ext] || 'application/octet-stream');
            res.end(fs.readFileSync(filePath));
            return;
        }
    ...
    if (imageExts.includes(ext)) {
        ...
        if (fs.existsSync(filePath)) {
            res.setHeader('Cache-Control', 'public, max-age=60, s-maxage=0'); // 7天缓存
