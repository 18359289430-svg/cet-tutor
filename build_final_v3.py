...
def extract_all_answers(content):
    ...
    for section in sections[1:]:
        ...
        for line in lines:
            ...
            # 匹配答案行
            ans_match = re.match(r'([A-D]）)?\s*【解析|精析】(.*)', line)
            if ans_match:
                # 向前查找选项
                pass
            
            # 匹配中文答案格式
            ans_match_cn = re.search(r'([A-D]）)\s*(?:【|.*?)(?:解|析)(.*)', line)
            if ans_match_cn:
                opt = ans_match_cn.group(1).replace('）', ')')
                explanation = ans_match_cn.group(2).strip()
            ...
            # 匹配中文题目格式: "1. 从男士的检查结果中..."
            q_match_cn = re.match(r'(\d+)\.\s*从|关于|为什么|如何|谁|什么|哪里|何时', line)
            if q_match_cn:
                q_num = int(q_match_cn.group(1))
...
def extract_long_conversations(content, year, month, set_num, passage_offset=0):
    ...
    # 匹配多种格式的Section A
    section_a = re.search(r'(?:Section A|Conversation One|长对话一)[^\n]*(?:Questions|\d+ to \d+)[^\n]*.*?(?=(?:Section B|Conversation Two|长对话二)|$)', content, re.DOTALL)
    
    if section_a:
        text = clean_text(section_a.group())
        
        # 提取听力原文
        script_pattern = r'(?:M:|W:|男:|女:)[^\n]+'
        scripts = re.findall(script_pattern, text)
        ...
        # 提取中文题目
        q_pattern_cn = r'(\d+)\.\s*((?:从|关于|为什么|如何|谁|什么|哪里|何时)[^\n？]*?)'
        q_matches_cn = re.findall(q_pattern_cn, text)
        ...
        for q_num, q_text in q_matches_cn:
            qid = f"{year}-{month:02d}-S{set_num}-LC-{int(q_num):02d}"
            ability = "关键信息捕捉"
            questions.append({
                "id": qid,
...
def extract_passages(content, year, month, set_num, passage_offset=0):
    ...
    # 匹配Section B或短文
    section_b = re.search(r'(?:Section B|Passage One|短文一|语篇)[^\n]*(?:Questions|\d+ to \d+)[^\n]*.*?(?=(?:Section C|Passage Two|语篇二)|$)', content, re.DOTALL)
    
    if section_b:
        text = clean_text(section_b.group())
        parts = re.split(r'(?:Questions \d+ to \d+|\d+\.)', text)
        ...
        for part in parts[1:]:
            if len(part) > 50:
                ...
                q_pattern_cn = r'(\d+)\.\s*((?:从|关于|为什么|如何|谁|什么|哪里|何时)[^\n？]*?)'
                q_matches_cn = re.findall(q_pattern_cn, part)
...
def extract_lectures(content, year, month, set_num, passage_offset=0):
    ...
    # 匹配Section C或讲座
    section_c = re.search(r'(?:Section C|Recording|讲座|语篇)[^\n]*(?:Questions|\d+ to \d+)[^\n]*.*?(?=Part III|Part IV|$)', content, re.DOTALL)
...
def extract_reading(content, year, month, set_num, passage_offset=0):
    ...
    # 查找阅读部分
    section_c = re.search(r'(?:Passage One|Passage Two|Reading Comprehension|阅读)[^\n]*.*?(?=Part IV|Part V|Translation|$)', content, re.DOTALL)
    ...
    if section_c:
        ...
        # 提取题目
        q_pattern = r'(\d{2})\.\s*((?:What|Why|How|Where|When|Who|Which|Whose|Whom)[^?\n]{10,100}\?)'
        q_matches = re.findall(q_pattern, text)
