...
\n\n`;
            ...
            // 五维分数详情（复用上方dimScores）
                ...
                if (isCompanion) {
                    ...
                    savePracticeRecord();
                    checkStreakOnChat();
                    detectAndGenerateAudio(fullText, bubbleEl);
                    try { var ct = JSON.parse(localStorage.getItem(examKey('current_training')) || 'null'); if (ct) trackTrainingCompletion(ct.skill, ct.stage, fullText); } catch(e) {}
                    // 最后一条免费消息时追加轻提示
...
function startPractice(type) {
    ...
    var skill = skillMap[type] || type;
    var stage = getTrainingStageForSkill(skill);
    startTraining(skill, stage);
}

function getTrainingStageForSkill(skill) {
    var skillLevels = getSkillLevelsForTraining();
    var info = skillLevels[skill];
    if (!info) return 1;
    var level = info.level || 1;
    var stageMap = { 1: 1, 2: 2, 3: 3, 4: 4 };
    var stage = stageMap[level] || 1;
    var currentStage = getCurrentStage(skill);
    return Math.max(stage, currentStage);
}

function getSkillLevelsForTraining() {
    try {
        var data = localStorage.getItem(examKey('ability_scores'));
        if (data) {
            ...
            if (scores.dims) {
                var result = {};
                var dimSkillMap = { listening: 'listening', reading: 'reading', writing: 'writing', translation: 'translation' };
                for (var k in scores.dims) {
                    var sk = dimSkillMap[k];
                    if (!sk) continue;
                    var s = scores.dims[k];
                    var lv, nm, cl;
                    if (s < 35) { lv = 1; nm = '薄弱'; cl = '#E17055'; }
                    else if (s < 60) { lv = 2; nm = '待提升'; cl = '#FDCB6E'; }
                    else if (s < 80) { lv = 3; nm = '良好'; cl = '#00B894'; }
                    else { lv = 4; nm = '冲刺'; cl = '#6C5CE7'; }
                    result[sk] = { score: s, level: { level: lv, name: nm, color: cl } };
                }
                return result;
            }
        }
...
function getNextAction(skillKey, skillInfo) {
    var stageNames = {
        listening: ['听前预判', '信号词捕获', '同义替换', '笔记速记', '全真模拟'],
        reading: ['定位速度', '同义替换', '推理判断', '长篇匹配', '全真模拟'],
        writing: ['语法清零', '逻辑框架', '自然表达', '完整写作'],
        translation: ['核心词汇', '拆句成短', '反中式英语', '完整段落']
    };
    var level = skillInfo.level.level;
    var stages = stageNames[skillKey] || [];
    var stageIdx = Math.min(level - 1, stages.length - 1);
    if (stages[stageIdx]) return '开始训练：' + stages[stageIdx];
    return '继续练习';
}
...
// ===== 听力训练音频生成 =====
function generateListeningAudio(text, callback) {
    fetchWithTimeout('/api/tts/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: text, exam_type: EXAM_TYPE || 'cet4' })
    }).then(function(resp) { return resp.json(); }).then(function(data) {
        if (data.url) callback(data.url);
        else { console.warn('[TTS] No URL:', data); callback(null); }
    }).catch(function(err) { console.warn('[TTS] Failed:', err); callback(null); });
}

function renderListeningAudio(bubbleEl, audioUrl) {
    if (!bubbleEl || !audioUrl) return;
    if (bubbleEl.querySelector('.listening-audio-player')) return;
    var d = document.createElement('div');
    d.className = 'listening-audio-player';
    var speedLabel = EXAM_TYPE === 'cet6' ? '六级语速1.2x' : '四级语速1.05x';
    d.innerHTML = '<div class="audio-player-header"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6C5CE7" stroke-width="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg><span class="audio-player-label">听力音频</span><span class="audio-speed-hint">' + speedLabel + '</span></div><audio controls preload="metadata" style="width:100%;height:36px;border-radius:6px;outline:none" src="' + audioUrl + '"></audio>';
    var first = bubbleEl.firstChild;
    if (first) bubbleEl.insertBefore(d, first); else bubbleEl.appendChild(d);
}
...
function detectAndGenerateAudio(fullText, bubbleEl) {
    ...
    if (match && match[1] && match[1].trim().length > 20) {
        ...
        loadingDiv.className = 'listening-audio-player audio-loading';
        loadingDiv.innerHTML = '<div class="audio-player-header"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6C5CE7" stroke-width="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg><span class="audio-player-label">听力音频生产中...</span></div><div class="audio-loading-bar"><div class="audio-loading-progress"></div></div>';
        var first = bubbleEl.firstChild;
        if (first) bubbleEl.insertBefore(loadingDiv, first); else bubbleEl.appendChild(loadingDiv);
        generateListeningAudio(listeningText, function(url) {
            var loadEl = bubbleEl.querySelector('.audio-loading');
            if (loadEl) loadEl.remove();
            if (url) renderListeningAudio(bubbleEl, url);
        });
    }
}
...
window.startFullDiagnosis = startFullDiagnosis;
